...
```bash
make deb        # dist/enigmars-utils_1.0.5_all.deb
make rpm        # dist/enigmars-utils-1.0.5-1.noarch.rpm  (needs rpmbuild)
make appimage   # dist/Enigmars_Utils-1.0.5-x86_64.AppImage
make pkg-arch   # packaging/arch/enigmars-utils-1.0.5-1-any.pkg.tar.zst
```
...
Release. Version tags (`v1.0.5`, …) get a matching named release.
...
sudo dpkg -i dist/enigmars-utils_1.0.5_all.deb
...
1. Tag a release matching `pkgver`:
   ...
   ```bash
   git tag -s v1.0.5 -m "Enigmars Utils 1.0.5"
